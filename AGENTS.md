...
- **List autoUnwrap mirrors Comark.** `listItemSpec.toComark` uses `autoUnwrapBlocks`: a single attrless paragraph flattens to inlines; a paragraph followed by a nested list keeps its wrapper (`['li',{},['p',{},'a'],['ul',…]]`). This matches comark's canonical form (verified on 0.6.2).
...
- **Picture markdown round-trips since comark 0.6** (inline directive form reparses; block form paragraph-wraps the img, which `pictureSpec.fromComark` re-absorbs). Remaining upstream renderer bug: a block `picture` directive rendered UNDER a `p` wrapper over-indents its body (4 spaces) so `:::source` reparses as an indented code block — we sidestep it via the sole-child hoist, but a paragraph carrying htmlAttrs around a lone picture still hits it. Worth filing upstream.
- **Streaming auto-close is off.** All `parseMarkdown()` calls pass `{ autoClose: false }` (serializer.ts `PARSE_OPTIONS`). comark's default closes dangling markers — right for streaming, wrong for the complete docs the editor parses. 0.6 narrowed it a lot (it now only fires on input NOT ending in a newline, and space-surrounded markers are ignored), but `"text with **partial"` — a complete doc without a trailing newline — still gains a `<strong>`. Do not re-enable it for whole-document parsing.
